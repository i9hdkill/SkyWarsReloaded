...
public class GameController {
	...
	private org.bukkit.material.Sign meteSign = new org.bukkit.material.Sign();
	...
	public void deleteGame(final int gameNumber) {
		...
		if (SkyWarsReloaded.getCfg().bungeeEnabled()  && !shutdown) {
			SkyWarsReloaded.get().getServer().getScheduler().runTaskLater(SkyWarsReloaded.get(), this::createGame, 40);
		}
		if (SkyWarsReloaded.getCfg().signJoinMode()  && !shutdown && gs != null) {
			SkyWarsReloaded.get().getServer().getScheduler().runTaskLater(SkyWarsReloaded.get(), () -> createGame(gameNumber, gs), 40);
		}
	}
	...
	public ArrayList<Game> getGames() {
		...
		games.addAll(gameNumbers.values());
		return games;
	}
